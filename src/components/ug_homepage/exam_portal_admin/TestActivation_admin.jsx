...
//       data.forEach(test => {
//         initialActivationStates[test.testCreationTableId] =test.status || false;;
//       });
//     } catch (error) {
//       console.error('Error fetching data:', error);
//     }
...
const TestActivation_admin = ({ testCreationTableId }) => {
  ...
  const handleDropdownChange = async (testId, value) => {
    ...
    if (testWithTotalQuestions) {
      try {
        const response = await fetch(`http://localhost:5001/updateTestActivationStatus/${testId}`, {
          method: 'POST',
          headers: {
      ...
      <div className='Test_Activation_Admin_conatiner'>
        ...
        <div className='Test_Activation_Admin_subconatiner'>
          {tests.map(test => {
            if (
              selectedStatus === 'all' ||
              (selectedStatus === 'active' && activationStates[test.testCreationTableId]) ||
              (selectedStatus === 'inactive' && !activationStates[test.testCreationTableId])
            ) {
              return (
                <div className='Test_Activation_Admin_ecahcard' key={test.testCreationTableId}>
                  <h3>{test.TestName}</h3>
